...
impl ConcreteOptimizer for GeneticAlgorithm {
    ...
    M: OptModelTrait<LEN_Y, LEN_P, LEN_B, LEN_X>,
  {
    ...
    // bounds
    let log10_bounds = self.get_bounds(problem);
    ...
    for n_gen in 0..self.max_gen {
      ...
      for ind in pop.iter() {
        ...
        let mut thread_problem = problem.clone();
        ...
        let handle = thread::spawn(move || {
          ...
          if thread_ind.0 == f64::INFINITY {
            let thread_f = thread_problem.obj(&thread_ind.1);
            thread_f
          } else {
            thread_ind.0
          }
      ...
      // sort individuals in descending order (fmax problem)
    ...
    OptResult::new(pop[0].1.clone(), problem.x_index.clone(), pop[0].0)
  }
}
